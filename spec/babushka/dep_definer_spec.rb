...
describe "source_template" do
  it "should be base_template" do
    Babushka::DepContext.source_template.should == Babushka::Dep.base_template
  end
end
...
describe Babushka::DepDefiner, '#define!' do
  ...
  it "should allow DepDefinitionError to bubble up" do
    ...
    }.should raise_error(Babushka::DepDefinitionError)
  end
...
describe "args" do
  ...
  context "without arguments" do
    it "should fail when called with unnamed args" do
      L{ dep('no args').with('a') }.should raise_error(Babushka::DepArgumentError, "The dep 'no args' accepts 0 arguments, but 1 was passed.")
    end
    it "should fail when called with named args that don't match" do
      L{ dep('no args').with(:a => 'a') }.should raise_error(Babushka::DepArgumentError, "The dep 'no args' received an unexpected argument :a.")
    end
  end
  context "with the wrong number of arguments" do
    it "should fail when called with the wrong number of unnamed args" do
      L{ dep('1 arg', :a).with('a', 'b') }.should raise_error(Babushka::DepArgumentError, "The dep '1 arg' accepts 1 argument, but 2 were passed.")
      L{ dep('2 args', :a, :b).with('a') }.should raise_error(Babushka::DepArgumentError, "The dep '2 args' accepts 2 arguments, but 1 was passed.")
    end
    it "should fail when called with named args that don't match" do
      L{ dep('1 arg', :a).with(:a => 'a', :b => 'b') }.should raise_error(Babushka::DepArgumentError, "The dep '1 arg' received an unexpected argument :b.")
      L{ dep('1 arg', :a).with(:a => 'a', :b => 'b', :c => 'c') }.should raise_error(Babushka::DepArgumentError, "The dep '1 arg' received unexpected arguments :b and :c.")
    end
  end
  context "with empty arguments" do
    subject { dep('2 args', :a, :b).with() }
    it "should populate the args with Parameter objects" do
      subject.args.map_values {|_,v| v.class }.should == {:a => Babushka::Parameter, :b => Babushka::Parameter}
    end
  ...
  context "with non-symbol names" do
    it "should be rejected, singular" do
      ...
      }.should raise_error(Babushka::DepArgumentError, %{The dep '2 args' received a non-symbol argument "a".})
    end
    it "should be rejected, plural" do
      L{
        dep('2 args', :a, :b).with('a' => 'a', 'b' => 'b')
      }.should raise_error(Babushka::DepArgumentError, %{The dep '2 args' received non-symbol arguments "a" and "b".})
    end
  end
  context "with incomplete named arguments" do
    ...
    it "should partially populate the args with Parameter objects" do
      subject.args.map_values {|_,v| v.class }.should == {:a => Babushka::Parameter}
    end
    ...
    it "should lazily create the missing parameter" do
      subject.context.define!.b.should be_an_instance_of(Babushka::Parameter)
...
describe "#on for scoping accepters" do
  ...
  before {
    Babushka.stub(:host).and_return(Babushka::OSXSystemProfile.new)
